...
void install_pkg(const char *file)
{
	...
	if (!is_pkg_type_supported(file)) {
		printScreenXY(0, 2, "Unsupported PKG type detected.", 0xFF);
	...
	for (i = 0; i < pspcount; i++) {
		...
		if (is_file[i]) {
			...
			if (!do_extract) {
				printScreenXY(0, 4, "Do you to extract this file?\n[] - Yes\n/\\ - No\nO - Yes to all", 0xFF00);

				while (!(pad.Buttons & (PSP_CTRL_SQUARE | PSP_CTRL_CIRCLE | PSP_CTRL_TRIANGLE)))
					sceCtrlPeekBufferPositive(&pad, 1);
			...
			if ((!do_overwrite) && (sceIoGetstat(path, &stat) >= 0)) {
				printScreenXY(0, 4, "Do you to overwrite this file?\n[] - Yes\n/\\ - No\nO - Yes to all", 0xFF00);
